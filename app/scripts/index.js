...
(function (RecoTwExplorer) {
    ...
    var RecoTwStatistics = (function () {
         ...
         */
        function RecoTwStatistics(enumerable) {
            this.users = enumerable.groupBy(function (x) { return x.target_id; }).select(function (x) { return ({ target_sn: x.firstOrDefault().target_sn, count: x.count() }); }).orderByDescending(function (x) { return x.count; }).thenBy(function (x) { return x.target_sn.toLowerCase(); }).toArray();
            ...
            this.dataTable = new google.visualization.DataTable({
                ...
                rows: this.users.map(function (x) { return ({ c: [{ v: x.target_sn }, { v: x.count }] }); })
            });
        }
    ...
    var Model = (function () {
        ...
        Model.RECOTW_GET_ALL_URL = "http://api.recotw.black/1/tweet/get_tweet_all";
        Model.RECOTW_POST_URL = "http://api.recotw.black/1/tweet/record_tweet";
        Model.RECOTW_COUNT_URL = "http://api.recotw.black/1/tweet/count_tweet";
